...
public class HttpServletUtils {
	...
	/**
	 * 获取request的请求参数。 格式：key=value&key=value
	 * 
	 * @param request request
	 * @return request的请求参数。
	 */
	 ...
	 * 以上是获取ip代码，另外Nginx还需要配置正确，如果Nginx反向代理有多台，那么在最外层的反向代理服务器加上 proxy_set_header
	 * X-Real-IP $remote_addr; proxy_set_header X-Forwarded-For $remote_addr;
	 * 内层的反向代理服务器加上 proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
	 * (！注意不要加X-Real-IP配置) 配置好后重启Nginx即可
